...
public class CarDao {
	...
	// 자동차 대여
	int rentCar(Connection conn, String carnumber) {
		...
		try {
			String sql = 
					"update car set rent = 1 where rent !=1 carnumber=? ";	 // 
...
//					e.printStackTrace();
	...
	// 자동차 반납
	int returnCar(Connection conn,  String carnumber) {
		...
		try {
			String sql = 
					"update car set rent=0 where rent !=0  and carnumber=?  ";		
