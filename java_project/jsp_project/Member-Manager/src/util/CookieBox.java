...
public class CookieBox {
	...
	// 이름으로 쿠키의값을 반환하는 메소드 (쿠키는 문자열 저장)
	...
	// Cookie 객체 생성하는 메소드 : 
	// 오버로딩 (같은 이름의 메소드를 만드는거) , static (필요할때 바로 만들어서 쓸수있도록)
	// 쿠키 이름, 쿠키 값
	...
	public static Cookie makeCookie(String name, String value, 
			String path, int maxAge, String domain) throws UnsupportedEncodingException {
		Cookie c = new Cookie(name,URLEncoder.encode(value, "utf-8"));
		c.setPath(path);//    /
		c.setMaxAge(maxAge);
		c.setDomain(domain);
